...
abstract class AssetPickerBuilderDelegate<Asset, Path> {
  ...
  SystemUiOverlayStyle get overlayStyle =>
      ...
      (theme.effectiveBrightness.isDark ? SystemUiOverlayStyle.light : SystemUiOverlayStyle.light);
  ...
  late final ValueNotifier<bool> permissionOverlayHidden = ValueNotifier<bool>(permission.value.isAuth);
...
class DefaultAssetPickerBuilderDelegate extends AssetPickerBuilderDelegate<AssetEntity, AssetPathEntity> {
  ...
  Future<void> _pushAssetToViewer(
    ...
    bool selectedPhotosAndIsVideo() =>
        isWeChatMoment && asset.type == AssetType.video && provider.selectedAssets.isNotEmpty;
  ...
  AssetPickerAppBar appBar(BuildContext context) {
    return AssetPickerAppBar(
      ...
      actions: (!isAppleOS || !isPreviewEnabled) && (isPreviewEnabled || !isSingleAssetMode)
      ...
      body: Consumer<DefaultAssetPickerProvider>(
        builder: (BuildContext context, DefaultAssetPickerProvider p, __) {
          ...
          final bool shouldDisplayAssets =
              p.hasAssetsToDisplay || (_specialItem != null && specialItemPosition != SpecialItemPosition.none);
  ...
  Widget assetsGridBuilder(BuildContext context) {
    return Selector<DefaultAssetPickerProvider, AssetPathEntity?>(
      ...
      builder: (_, AssetPathEntity? path, __) {
        ...
        if (specialItemPosition != SpecialItemPosition.none && path?.isAll == true) {
          totalCount += 1;
        }
  ...
  Widget assetGridItemBuilder(
    ...
    if (specialItem != null) {
      if ((index == 0 && specialItemPosition == SpecialItemPosition.prepend) ||
          (index == _length && specialItemPosition == SpecialItemPosition.append)) {
        return specialItem;
      }
    }
          ...
          builder: (_, DefaultAssetPickerProvider p, __) {
            ...
            return Semantics(
              ...
              child: GestureDetector(
                ...
                onLongPress: isPreviewEnabled && context.mediaQuery.accessibleNavigation
                ...
                if (p != null)
                  Flexible(
                    child: ScaleText(
                      ...
                      maxScaleFactor: 1.2,
                      semanticsLabel:
                          isPermissionLimited && p.isAll ? semanticsTextDelegate.accessiblePathName : p.name,
                    ),
                  ),
                w!,
              ],
  ...
  Widget pathEntityWidget({
    ...
    final String name = isPermissionLimited && pathEntity.isAll ? textDelegate.accessiblePathName : pathEntity.name;
    final String semanticsName =
        isPermissionLimited && pathEntity.isAll ? semanticsTextDelegate.accessiblePathName : pathEntity.name;
  ...
  Widget previewButton(BuildContext context) {
    Future<void> _onTap() async {
      ...
      final List<AssetEntity> _selected;
      if (isWeChatMoment) {
        _selected = _selectedAssets.where((AssetEntity e) => e.type == AssetType.image).toList();
      } else {
